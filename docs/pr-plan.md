...
| 11 | feat: 增加账号登录和账号级日程保留 | 手机号、密码、短信验证码演示登录入口、账号专属 LocalStorage 日程空间 | 登录两个手机号分别添加日程，切换后检查日程隔离 |
| 12 | feat: 增加删除前确认 | 删除命令先展示候选，确认或选择序号后再删除 | 输入“取消明天开会”，再输入“第 1 个”或“确认删除” |
| 13 | feat: 增加 Android 原生语音识别 App | Capacitor Android 工程、语音 adapter、GitHub Release workflow | `npm test`、`npm run cap:sync`、GitHub Actions 构建 APK |
