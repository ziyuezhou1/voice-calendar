...
| 11 | feat: 增加修改前候选确认 | 修改命令命中多条相似日程时先展示候选，选择序号后再修改 | 输入“把明天开会改到四点”，再输入“第 2 个” |
| 12 | feat: 增加账号登录和账号级日程保留 | Google/微信演示登录入口、账号专属 LocalStorage 日程空间 | 登录两个账号分别添加日程，切换后检查日程隔离 |
| 13 | feat: 增加删除前确认 | 删除命令先展示候选，确认或选择序号后再删除 | 输入“取消明天开会”，再输入“第 1 个”或“确认删除” |
| 14 | feat: 增加 Android 原生语音识别 App | Capacitor Android 工程、语音 adapter、GitHub Release workflow | `npm test`、`npm run cap:sync`、GitHub Actions 构建 APK |
| 15 | docs: 补充依赖和原创说明 | `README.md`、PR 模板 | 检查 README 与 PR 描述完整性 |
